...
are collections of variables that serve as input for a FORTRAN code.
...
We are using the pseudo potential file (`Si.pz-vbc.UPF`) downloaded from [Quantum
...
This how we run `pw.x` in self consistent mode for silicon in the terminal.
...
Make sure that you have added the Quantum ESPRESSO executable directory to the `PATH` environment
variable in bash/zsh profile, otherwise we have to type the full path of `pw.x`
executable location. If you installed Quantum ESPRESSO via APT, this is already taken care of.
...
Create a jupyter notebook (you can call it `tasks.ipynb`) to setup your calculation inside of the folder `calculations` of this tutorial.

Mount your Google Drive with:
...
Navegate into the `calculations` folder of the tutorial with:
...
Now, try for yourself to do the next steps in the jupyter notebook.

Inside of `calculations`, create a folder called `task 1`.
Copy to it the QE input file `pw.scf.silicon.in` that you will find in `scr/silicon`.

Create a function that search for a keyword in a file, and updates the value of the variable.
Update `pseudo_dir` giving the correct path, which is in `../../src/pseudos`.

Now run the QE scf calculation running `pw.x`.

Then, read the output file of the QE scf calculation and print the information that you have seen above: energies, exchange-correlation, k-points, Kohn-Sham states, ... You can search for keywords like (but also check your output file):

"!    total energy ", "Exchange-correlation", "number of k points", "Kohn-Sham states", "G-vector sticks info" 

Try to do a function to search for a keyword and print a few lines before and/or after the keyword.

:::tip Task-solutions

Inside of the directory `notebooks`, we are providing `tasks-solutions.ipynb`. You can use it to help yourself building your own solution to the problems. However, please avoid just copying and pasting it. Before using the solutions, we would recommend you to try searching for help with an AI tool, such as DeepSeek or ChatGPT.
