...
Create a jupyter notebook (you can call it `task1.ipynb`) to setup your calculation inside of the folder `calculations` of this tutorial.
...
Update `pseudo_dir` giving the correct path, which is in `../scr/pseudo`.

Now run the QE scf calculation running `pw.x`.

Then, read the output file of the QE scf calculation and print the information that you have seen above: energies, exchange-correlation, k-points, Kohn-Sham states, ... You can search for keywords like:
